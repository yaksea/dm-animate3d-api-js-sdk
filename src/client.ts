...
import { ProgressCallback, ResultCallback, ProgressCallbackData, ResultCallbackData, JobResult, JobError } from './data/callback';
import { AuthenticationError, APIError, ValidationError, TimeoutError } from './exceptions';
import { ends_with_mp_tracked_id, is_http_url, get_file_extension, get_file_name_without_ext, file_exists, ensure_directory_exists } from './utils';
...
export class Animate3DClient {
  ...
  async upload_character_model(
    ...
    if (is_http_url(source)) {
      ...
      const upload_urls = await this._get_model_upload_url(name, model_ext);
      await this._upload_file_to_gcs(upload_urls.modelUrl, source);

      return await this._store_model(
        upload_urls.modelUrl,
        name,
        create_thumb
      );
    }
  ...
  private async _get_model_upload_url(name: string, model_ext: string): Promise<{ modelUrl: string }> {
    const params = {
      name,
      modelExt: model_ext,
      resumable: '0'
